...
def connect_db():
    connection = None
    try:
        connection = pymysql.Connect(host = "localhost",user = 'root',password = 'Priy@dip',database="priyo_sic",port=3306,charset = 'utf8')
...
def create_db():
    query = 'create database IF NOT EXISTS priyo_sic'
...
def create_table():
    query = 'create table IF NOT EXISTS employees1(id int primary key auto_increment, name varchar(50) not null, designation varchar(30), phone_number	bigint unique, salary float, commission		float default(0), years_of_experience tinyint, technology		varchar(30)	not null)'
    connection = connect_db()
    try:
        cursor = connection.cursor()
        cursor.execute(query)
...
def read_employee_details():  #make this somehow to be used by insert and update employee conditonally , pass 1 &
...
def insert_employee(): #pass here 1 
...
def update_employee(): #pass here 2 
    salary = float(input('Enter employee salary: '))
    years_of_experience = input('Enter employee years of experience: ')
